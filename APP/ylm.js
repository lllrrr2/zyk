...
let update_data = "完成签到，优豆查询";
//=======================================//
const $ = new Env('优乐米');
const notify = $.isNode() ? require('./sendNotify') : '';
const Notify = 1 		//0为关闭通知,1为打开通知,默认为1
let UserCookie = ($.isNode() ? process.env.ylm_data : $.getdata('ylm_data')) || '';
let UserCookieArr = [];
const {log} = console;
let data = '';
let msg =``;
var timestamp = Math.round(new Date().getTime()).toString();
let host = 'admin.cdylm.com';
...
function getMemberInfo(task,timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/api/center/center`,
            headers: {
                Host: host,
                'Host': host,
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 200) {
                    DoubleLog(`\n 会员信息: ✅ ，用户：${result.data.nickname} 当前会员为 ${result.data.user_level_name}e${result.data.bean} 优豆`)
				} else if (task == 2) {
					DoubleLog(`\n 会员查询: ✅ ，当前会员为 ${result.data.user_level_nam}`);
                } else {
