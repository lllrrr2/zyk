...
请求文本：token
变量
export ydwxhd='deviceParams&token'
多号@隔开
*/
const $ = new Env('一点万象');
...
let ydwxhd = ($.isNode() ? process.env.ydwxhd : $.getdata("ydwxhd")) || ""
let ydwxhdArr = [];
let data = '';
let msg = '';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            log(`\n============ 微信公众号：老司机上线 ============`)
            log(`\n=================== 共找到 ${ydwxhdArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${ydwxhdArr}`);
            }
            for (let index = 0; index < ydwxhdArr.length; index++) {

                let num = index + 1
                addNotifyStr(`\n==== 开始【第 ${num} 个账号】====\n`, true)

                ydwxhd = ydwxhdArr[index];
                ydwxhd=ydwxhd.split('&')            
...
async function checkin() {
  ...
  t = timestamp
  sings =MD5('action=mixc.app.memberSign.sign&apiVersion=1.0&appId=68a91a5bac6a4f3e91bf4b42856785c6&appVersion=3.32.0&deviceParams='+ydwxhd[0]+'&imei=2333&mallNo=20014&osVersion=12.0.1&params=eyJtYWxsTm8iOiIyMDAxNCJ9&platform=h5&timestamp='+t+'&token='+ydwxhd[1]+'&P@Gkbu0shTNHjhM!7F')      
  var options = {
  method: 'POST',
...
data:'mallNo=20014&appId=68a91a5bac6a4f3e91bf4b42856785c6&platform=h5&imei=2333&appVersion=3.32.0&osVersion=12.0.1&action=mixc.app.memberSign.sign&apiVersion=1.0&timestamp='+t+'&deviceParams='+ydwxhd[0]+'&token='+ydwxhd[1]+'&params=eyJtYWxsTm8iOiIyMDAxNCJ9&sign='+sings
};
    if (debug) {
            log(`\n【debug】=============== 这是  请求 url ===============`);
        ...
        axios.request(options).then(async function(response) {
            try {
                ...
                if(data.code=0){
                    log('point:'+data.data.point)
                    log('point:'+data.data.userPoints)
