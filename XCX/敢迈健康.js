 ...
 * 项目类型：微信小程序
 * 项目名称：敢迈健康+
 * 项目抓包：抓api.digital4danone.com.cn下的X-Access-Token & openId & unionId填入变量
 * 项目变量：gmjk_Cookie
 * 项目定时：每40分钟运行一次
 * cron: 0 0,7 * * *
...
let update_data = "小程序任务要求：改成这四个 记录便便  按时吃饭  23点前睡觉  起床喝杯水";
//=======================================//
const $ = new Env('敢迈健康');
...
let UserCookie = ($.isNode() ? process.env.gmjk_Cookie : $.getdata('gmjk_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
var timestamp = Math.round(new Date().getTime()).toString();
let host = 'api.digital4danone.com.cn';
...
function getMemberInfo(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/healthyaging/danone/wx/ha/haUser/info`,
            headers: {
                Host: host,
        ...
        $.get(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 200) {
                    ...
                    DoubleLog(`\n 会员信息: ✅ ，用户${result.result.mobile}`)
                } else {
                    DoubleLog(`\n 会员信息: ❌ ，原因是：${result.message}`)
                }
            } catch (e) {
                DoubleLog(`\n 信息异常: ❌ ，${response}`)
            } finally {
 ...
 * @returns {Promise<unknown>}
 */
function report(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/healthyaging/danone/wx/config/eventReport`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
            ...
            body: `{"content":"选择挑战任务页-浏览","name":"maievent-page-view","type":"view","mobile":"${mobile}","openId":"${ck[1]}","unionId":"${ck[2]}","page":"/sub1/pages/challenge-select/challenge-select?type=restart","source":"wechat-default","sdk":"ha-default"}`,
        }
 ...
 * @returns {Promise<unknown>}
 */
function getUserTask(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/healthyaging/danone/wx/clockin/getUserTask`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
                ...
                if (result.code == 200) {
                    if(result.result[0].status == 1){
                        ...
                        if(result.result[0].ruleList != false){
                            ...
                            ruleId1 = result.result[0].id ;
                        }
                        DoubleLog(`\n 获取任务: ✅ ，${result.result[0].simpleName} 未完成`);
                        await clickIn(ruleId1,taskid1,2 * 1000);
                        await $.wait(5000);
				    }else{
                        DoubleLog(`\n 获取任务: ❌ ，${result.result[0].simpleName} 已完成`)
			 	    }
                    if(result.result[1].status == 1){
                        ...
                        if(result.result[1].ruleList != false){
                            ...
                            ruleId2 = result.result[1].id ;
                        }
                        DoubleLog(`\n 获取任务: ✅ ，${result.result[1].simpleName} 未完成`);
					    await clickIn(ruleId2,taskid2,2 * 1000);
                        await $.wait(5000);
				    }else{
                        DoubleLog(`\n 获取任务: ❌ ，${result.result[1].simpleName} 已完成`)
			 	    }
                    if(result.result[2].status == 1){
                        ...
                        if(result.result[2].ruleList != false){
                            ...
                            ruleId3 = result.result[2].id ;
                        }
                        DoubleLog(`\n 获取任务: ✅ ，${result.result[2].simpleName} 未完成`);
					    await clickIn(ruleId3,taskid3,2 * 1000);
                        await $.wait(5000);
				    }else{
                        DoubleLog(`\n 获取任务: ❌ ，${result.result[2].simpleName} 已完成`)
			 	    }
                    if(result.result[3].status == 1){
                        ...
                        if(result.result[3].ruleList != false){
                            ...
                            ruleId4 = result.result[3].id ;
                        }
                        DoubleLog(`\n 获取任务: ✅ ，${result.result[3].simpleName} 未完成`);
					    await clickIn(ruleId4,taskid4,2 * 1000);
				    }else{
                        DoubleLog(`\n 获取任务: ❌ ，${result.result[3].simpleName} 已完成`)
			 	    }
                } else {
                    DoubleLog(`\n 获取任务: ❌ ，原因是：${result.message}`)
                }
            } catch (e) {
                DoubleLog(`\n 信息异常: ❌ ，${response}`)
            } finally {
 ...
 * @returns {Promise<unknown>}
 */
function clickIn(ruleId,taskid,timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/healthyaging/danone/wx/clockin/clickIn`,
            headers: {
                Host: host,
                'Connection': 'keep-alive',
                'User-Agent': getUA(),
            ...
            body: '{"ruleIds : [ ' + ruleId + ' ],taskDataCode" : "Auto","taskDataValue" : null,"userTaskDetailId" :' + taskid + '}', 
        }
        $.post(url, async (error, response, data) => {
            try {
                let result = JSON.parse(data);
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_mnflag_Cookie`)
        return;
    }
    return true;
}
