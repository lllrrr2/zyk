...
let UserCookie = ($.isNode() ? process.env.gjjjhyjlb_Cookie : $.getdata('gjjjhyjlb_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let ck, ck_status;
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (task == 1) {
                    ...
                    DoubleLog(`\n 登录成功: ✅ ，会员：${result.mobile}，当前 ${result.point} 积分`)
				} else if (task == 2) {
					DoubleLog(`\n 积分查询: ✅ ，签到后有积分 ${result.point}`);
                } else {
                    DoubleLog(`\n 登录失败: ❌ ，原因是：${data}`)
                }
            } catch (e) {
    ...
    if ($request.url.indexOf("club-server/front/member/home") > -1) {
		...
		if (ckStr) {
			if (ckStr.indexOf(ck_id) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "gjjjhyjlb_Cookie");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck}`);
			}
		} else {
			$.setdata(ck, "gjjjhyjlb_Cookie");
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 gjjjhyjlb_Cookie`)
        return;
    }
    return true;
}
