 ...
 !(async () => {
     ...
     for (let i = 0; i < cookiesArr.length; i++) {
       if (cookiesArr[i]) {
         ...
         console.log(`\n******开始【🐳爱海盐账号${$.index}】${$.nickName}|${$.mobile}*********\n`);
         await main()
       }
     }
     if ($.isNode() && message) {
  ...
  */
  async function credential_auth() {
   let url = {
     ...
     body: `client_id=10018&password=${encodeURIComponent($.pwd)}&phone_number=${$.mobile}`,
     headers: {
