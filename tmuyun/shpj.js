...
变量：shpjhd 例： X-SESSION&X-REQUEST-ID  多个账号使用@或者换行隔开
*/
const axios = require('axios');
let request = require("request");
const $ = new Env('诗画浦江');

let shpjhd = ($.isNode() ? process.env.shpjhd : $.getdata("shpjhd")) || ""
let shpjhdArr = [];
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            log(`\n============ 微信小程序：柠檬玩机 ============`)
            log(`\n=================== 共找到 ${shpjhdArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${shpjhdArr}`);
            }
            for (let index = 0; index < shpjhdArr.length; index++) {

                let num = index + 1
                log(`\n==== 开始【第 ${num} 个账号】====\n`)
                shpjhd = shpjhdArr[index];
                sessionId = shpjhd.split('&')[0]
                requestId = shpjhd.split('&')[1]
...
async function GetNews() {
    ...
    return new Promise((resolve) => {
        var options = {
            method: "get",
            url: `${url}/article/channel_list?channel_id=5cc02969b1985017d6fef804&isDiFangHao=false&is_new=true&list_count=0&size=50`,
            headers: {
                "User-Agent": "Mozilla/5.0 (Linux; Android 9; MI 9 Build/PKQ1.181121.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/4435 MMWEBSDK/20221206 Mobile Safari/537.36 MMWEBID/6710 MicroMessenger/8.0.32.2300(0x2800205D) WeChat/arm64 Weixin NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx489f950decfeb93e",
                'X-SESSION-ID': sessionId,
                "X-REQUEST-ID": requestId,
...
async function Envs() {
    if (shpjhd) {
        if (shpjhd.indexOf("@") != -1) {
            shpjhd.split("@").forEach((item) => {
                shpjhdArr.push(item);
            });
        } else if (shpjhd.indexOf("\n") != -1) {
            shpjhd.split("\n").forEach((item) => {
                shpjhdArr.push(item);
            });
        } else {
            shpjhdArr.push(shpjhd);
        }
    } else {
        log(`\n 【${$.name}】：未填写变量 shpjhd`)
        return;
